...
sf::VideoMode size=sf::VideoMode(sf::Vector2u(Desktop.x/5,Desktop.y/20));
sf::RenderWindow window(size,"test",sf::Style::Close);
...
int main(){
    ...
    //
    drbar dba;
    dba.setPosition(sf::Vector2f(size.size.y*3/2.0,size.size.y*3/8.0));
    dba.setColorDr(sf::Color(0,0,255));
    dba.setRadiusDr(size.size.y/8.0);
    dba.setColorBar(sf::Color(120,120,120));
    dba.setSizeBar(sf::Vector2f(size.size.x*3/4.0,size.size.y/4.0));
    //
    ...
    while (1)
    {
        ...
        // }
            ...
            if(dba.touching(sf::Mouse::getPosition(window))){
                dba.track=1;
